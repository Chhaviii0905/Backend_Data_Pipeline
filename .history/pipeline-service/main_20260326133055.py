...
def get_customers(page: int = 1, limit: int = 10, db: Session = Depends(get_db)):
    try:
        ...
        result = db.execute("SELECT COUNT(*) FROM customers").scalar()
        total = result if result else 0
        
        start = (page - 1) * limit
        customers = db.execute(f"SELECT customer_id, first_name, last_name, email, phone, address, date_of_birth, account_balance, created_at FROM customers ORDER BY customer_id LIMIT {limit} OFFSET {start}").fetchall()
...
def get_customer(customer_id: str, db: Session = Depends(get_db)):
    try:
        customer = db.execute("SELECT customer_id, first_name, last_name, email, phone, address, date_of_birth, account_balance, created_at FROM customers WHERE customer_id = %s", (customer_id,)).fetchone()
        if not customer:
